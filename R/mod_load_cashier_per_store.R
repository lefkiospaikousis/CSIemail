#' load_cashier_per_store UI Function
#'
...
mod_load_cashier_per_store_ui <- function(id){
  ns <- NS(id)
  tagList(
...
#' load_cashier_per_store Server Functions
#'
#' @noRd 
mod_load_cashier_per_store_server <- function(id){
  moduleServer( id, function(input, output, session){
    ns <- session$ns
    
    statement_type <- 'cashier_per_store'
    ...
    callModule(mod_downloadTable_server, id = "down_statement", table_name = "CASHIER_PER_STORE_STATEMENT", statement)
    ...
    statement <- reactive({
      ...
      dta <- tryCatch({
        ...
        read_monitoring_statement(file$datapath)
...
# mod_load_cashier_per_store_ui("load_cashier_per_store_1")

## To be copied in the server
# mod_load_cashier_per_store_server("load_cashier_per_store_1")
